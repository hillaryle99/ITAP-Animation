...
function loopThrough(arr, arr2) {
  ...
  code.innerHTML = `<br /><br />if(arr1.length != arr2.length) return False; <br> else <br> for (int i = 0; i < arr1.length; i++) <br> if (arr1[i] != arr2[i]) return False; <br> return True;`;
  nextButton.onclick = () => {
    console.log(i);
    console.log(j);
    ...
    if(currentArr.length != currentArr2.length){
      indexInfo.innerHTML = `<br><br> <span style='background-color:red'><b>Two arrays are not equal.</b></span> <br> Length of Array_1 is <span style='background-color:yellow'><b> ${currentArr.length}</b></span> <br> Length of Array_2 is <span style='background-color:yellow'><b> ${currentArr2.length} </b></span>`;
      document.getElementById('icon-button').style.display = 'none';
      ...
      if (lastArrayElement[0] != undefined) {
        if (arr[i] == undefined) {
          indexInfo.innerHTML = `<br><br><span style='background-color:yellow'><b> TWO ARRAYS ARE EQUAL <b></span>`;
          document.getElementById('icon-button').style.display = 'none';
          nextButton.disabled = true;
          document.getElementById('restart-loop').style.display = 'block';
        }
